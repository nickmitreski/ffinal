...
const FlashForwardFolder: React.FC<AppContentProps> = ({ onOpenApp }) => {
  ...
  const handleItemClick = (item: FolderItem) => {
    ...
    if (item.isBuiltIn && item.appId) {
      ...
      } else if (item.appId === 'servicesWindow') {
        // Open Services at a slightly different position
        onOpenApp(
          item.appId,
          undefined,
          item.name,
          { x: 150 + Math.floor(Math.random() * 40) - 20, y: 80 + Math.floor(Math.random() * 40) - 20 }
        );
      } else if (item.appId === 'pricingWindow') {
        // Open Pricing at a slightly different position
        onOpenApp(
          item.appId,
          undefined,
          item.name,
          { x: 200 + Math.floor(Math.random() * 40) - 20, y: 120 + Math.floor(Math.random() * 40) - 20 }
        );
      } else if (item.appId === 'contactUsWindow') {
        // Open Contact Us at a slightly different position
        onOpenApp(
          item.appId,
          undefined,
          item.name,
          { x: 250 + Math.floor(Math.random() * 40) - 20, y: 160 + Math.floor(Math.random() * 40) - 20 }
        );
      } else if (item.appId === 'ourWork') {
        // Open Our Work at a slightly different position
        onOpenApp(
          item.appId,
          undefined,
          item.name,
          { x: 300 + Math.floor(Math.random() * 40) - 20, y: 200 + Math.floor(Math.random() * 40) - 20 }
        );
      } else {
